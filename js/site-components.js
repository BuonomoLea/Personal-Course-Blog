...
class SiteHeader extends HTMLElement {
  ...
  async connectedCallback() {
    ...
    try {
      ...
      const res  = await fetch('Personal-Course-Blog/global/header.html', { cache: 'no-cache' });
...
class SiteFooter extends HTMLElement {
  async connectedCallback() {
    try {
      ...
      const res  = await fetch('Personal-Course-Blog/global/footer.html', { cache: 'no-cache' });
      const text = await res.text();
